...
class Training(db.Model):
        ...
        trainingEnd= db.Column(db.DateTime, nullable=False)    
        resourceUser = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
        maxPhishingMail = db.Column(db.Integer, default=0)
        
        user= db.relationship('User',backref=db.backref('training',lazy=True))

        def __init__(self, trainingName, trainingDesc, trainingStart,trainingEnd,resourceUser,maxPhishingMail):
            self.trainingName = trainingName
            self.trainingDesc = trainingDesc
            self.trainingStart = datetime.strptime(trainingStart,"%Y-%m-%d").date()
            self.trainingEnd = datetime.strptime(trainingEnd,"%Y-%m-%d").date()
