...
class PhishingEvent:
    ...
    def generate_phishing_link(self, user_id, training_id):
        """
        피싱 링크 생성 후, 링크를 기록하고 반환합니다.
        """
        # 유저 정보 조회
        ...
        if user:
            ...
            # user_event_log에 링크 생성 정보 기록
            self.log_event(user, phishing_link, event_type="link_generated", timestamp=timestamp_kst)

            return phishing_link, 200

        return not_found(f"User with ID {user_id} not found")

    def log_event(self, user, phishing_link, event_type, timestamp):
        """
        user_event_log 테이블에 피싱 링크 생성 또는 클릭 이벤트 기록.
        """
    ...
    def log_click_event(self, user_data):
        ...
        event_log = UserEventLog(
            ...
            timestamp=timestamp_kst,  # 타임스탬프 추가
